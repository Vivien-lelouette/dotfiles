...
Variable controlling whether window rules open buffers in new frames (ideal with tiling WMs) or keep them in the current frame.
Set at daemon launch: =emacs --daemon --eval '(setq frame-centric t)'=
...
Ensure symbols and emoji render with consistent metrics by mapping
Unicode ranges to appropriate fonts. JetBrains Mono covers box-drawing,
braille, arrows, etc. Noto Sans Symbols handles emoji-style characters
(❌ ✅) that JetBrains Mono lacks.
...
** Company-wordfreq
Prose autocompletion using word frequency dictionaries (FR + EN).
Run =M-x company-wordfreq-download-list= for =fr= and =en= to fetch dictionaries.
#+BEGIN_SRC emacs-lisp
...
** Agent Shell
Pour l'usage avec Claude, il faut avoir `claude-code` d'installé et `claude-code-acp`.
#+BEGIN_SRC emacs-lisp
...
** Gnus group buffer
Don't auto-subscribe to new IMAP groups, only subscribe to groups matching =INBOX=. Enable topic mode for organizing groups.
#+BEGIN_SRC emacs-lisp
...
** Gnus summary buffer
Most recent messages first, with a date column that adapts to recency.
#+BEGIN_SRC emacs-lisp
...
** Gnus article buffer
Display these headers in this order.
#+BEGIN_SRC emacs-lisp
  (setq
...
** Gnus storage
Don't use =.newsrc=, don't cache articles locally.
#+BEGIN_SRC emacs-lisp
...
** Auto-refresh
Check for new mail every 5 minutes and show indicator in doom-modeline.
#+BEGIN_SRC emacs-lisp
  (require 'gnus)
  (require 'gnus-demon)
...
** BBDB contact completion
Silently collect addresses from received mail, and provide company-mode completion when composing.
#+BEGIN_SRC emacs-lisp
  (use-package bbdb
    :config
...
* Google Calendar
Fetch Google Calendar events via the secret ICS URL and convert them to org entries. The ICS URL is stored in =local.el= to avoid committing it.

Set =gcal/ics-url= and =gcal/org-file= in =local.el=:
#+BEGIN_EXAMPLE
  (setq gcal/ics-url "https://calendar.google.com/calendar/ical/.../basic.ics"
        gcal/org-file "~/org/gcal.org")
#+END_EXAMPLE
...
** Timeblock view
Visual timeblock view of the calendar, similar to Google Calendar.
#+BEGIN_SRC emacs-lisp
