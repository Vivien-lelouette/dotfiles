    ...
    (let ((parent-frame (frame-parameter frame 'parent-frame)))
      ...
      (if parent-frame
          (let* ((parent-frame-pos (frame-position parent-frame))
                 ...
                 (absolute-x (if (< frame-pos-x parent-frame-pos-x) (+ parent-frame-pos-x frame-pos-x) frame-pos-x))
                 (absolute-y (if (< frame-pos-y parent-frame-pos-y) (+ parent-frame-pos-y frame-pos-y) frame-pos-y)))
            (set-frame-parameter frame 'left absolute-x)
            (set-frame-parameter frame 'top absolute-y))))
    frame)
