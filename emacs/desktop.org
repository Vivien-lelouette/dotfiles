...
#+begin_src emacs-lisp
  ...
  (use-package ewm
    ...
    (defun ewm/refresh-setup ()
      ...
      (defun theme/apply-window-divider ()
        ...
        (let ((color (if (cl-intersection '(dracula dracula-light) custom-enabled-themes)
          ...
          (set-face-attribute 'tab-bar-tab nil
                              :background color
                              :foreground (or (face-background 'cursor nil nil)
      ...
      (defun theme/apply-window-divider-on-theme (&rest _)
        "Run window-divider setup after a theme is enabled."
        (theme/apply-window-divider))
      (add-hook 'enable-theme-functions #'theme/apply-window-divider-on-theme)
...
* Vivaldi integration
Brotab (=bt=) provides the active tab's ID, title, and URL via =bt query +active=.
...
* CUA-style key swap
Swap C-x/C-w, C-c/M-w and C-v/C-y globally via =key-translation-map=.
This makes C-x cut, C-c copy and C-v paste everywhere, while the
original prefix keys move to C-w and M-w.
