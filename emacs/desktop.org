#+title Desktop with EWM configuration
...
* Window Management
** Window layouts
Persistent window layout slots: save/restore window arrangements across sessions using =window-state-get/put= and =savehist=.
...
* EWM Configuration
We use [[https://codeberg.org/ezemtsov/ewm][EWM]] (Emacs Wayland Manager) as the basis for our Emacs Desktop Environment.  EWM is a Wayland compositor that runs inside Emacs as a dynamic module, so applications never freeze waiting for Elisp evaluation.
...
** Vivaldi
  ...
  (defun vivaldi/get-url ()
    (interactive)
    (if (string= ewm-surface-app "Vivaldi-stable")
        (concat "http" (car (last (mapcar (lambda (str) (s-replace " - Vivaldi" "" str)) (split-string ewm-surface-title " - http")))))))

  (defun vivaldi/do-start-with-url-or-search ()
    (interactive)
    (if (string= ewm-surface-app "Vivaldi-stable")
        (vivaldi/start-with-url-or-search (completing-read "Vivaldi URL or search " vivaldi/input-history nil nil (vivaldi/get-url) 'vivaldi/input-history))
      (vivaldi/start-with-url-or-search (completing-read "Vivaldi URL or search " vivaldi/input-history nil nil nil 'vivaldi/input-history))))

  (defun vivaldi/start-with-url-or-search (input)
    (interactive)
    (let ((trimmed-input (string-trim input)))
      (if (string-match-p " " trimmed-input)
          (shell/async-command-no-output (concat "vivaldi-stable --new-window '? " input "'"))
        (shell/async-command-no-output (concat "vivaldi-stable --new-window '" input "'")))))

  (defun bookmark/vivaldi-bookmark-handler (record)
    "Jump to a Vivaldi bookmarked location."
    (with-current-buffer (window-buffer)
      (vivaldi/start-with-url-or-search (bookmark-prop-get record 'location))))

  (defun bookmark/vivaldi-bookmark-make-record ()
    "Return a bookmark record for the current Vivaldi buffer."
    (interactive)
    (let ((split-title (mapcar (lambda (str) (s-replace " - Vivaldi" "" str)) (split-string ewm-surface-title " - http"))))
      `(,(concat "vivaldi/" (car split-title))
  ...
  (defun bookmark/vivaldi-set-bookmark-handler ()
    ...
    (if (string= ewm-surface-app "Vivaldi-stable")
        (set (make-local-variable 'bookmark-make-record-function)
             #'bookmark/vivaldi-bookmark-make-record)))
