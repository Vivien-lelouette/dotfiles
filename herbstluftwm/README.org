...
#+BEGIN_SRC shell
   ...
   wpg -i .wallpaper ~/.cache/wal/colors.json &
   wpg -s .wallpaper &
   gsettings set org.gnome.desktop.background picture-options 'none' &
   gsettings set org.gnome.desktop.background primary-color "$BG_COLOR" &
   bash ~/.config/rofi/generate_themes.sh &
#+END_SRC
...
#+BEGIN_SRC shell
  ...
  XMODIFIERS=@im=none herbstclient keybind $Mod+Control+e spawn emacsclient -c
  herbstclient keybind $Mod+e spawn bash $herbstluftwm_scripts_folder/switch_by_classes.sh 1 'Emacs' '' 'XMODIFIERS=@im=none emacsclient -c'
  herbstclient keybind $Mod+Shift+e spawn bash $herbstluftwm_scripts_folder/switch_by_classes.sh -1 'Emacs' '' 'XMODIFIERS=@im=none emacsclient -c'
  herbstclient keybind $Mod+Return spawn bash $herbstluftwm_scripts_folder/emacs_command.sh "(call-interactively #'eshell)"
  ...
  herbstclient keybind $Mod+Control+Return spawn bash -c 'RULES="pseudotile=on focus=on" XMODIFIERS=@im=none '$herbstluftwm_scripts_folder'/spawn_with_rules.sh emacsclient --eval "(emacs-everywhere)"'
#+END_SRC
